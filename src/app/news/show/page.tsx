...
export default function ShowPage() {
  ...
  const { data: newsData, isLoading } = useQuery({
    queryKey: ['news', 'show', page],
    queryFn: async () => {
      const response = await axios.get(`/api/news/show?page=${page}`);
  ...
  const voteMutation = useMutation({
    ...
    onSuccess: (result, variables) => {
      queryClient.setQueryData(['news', 'show'], (oldData: any) => {
      ...
      <main className="belowHeaderContainer">
        <div className="p-4">
          ...
          {!isLoading && (!news || news.length === 0) && (
            <div className="text-muted mt-4">No show news found.</div>
          )}
