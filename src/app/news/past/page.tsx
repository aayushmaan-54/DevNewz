...
export default function PastPage() {
  ...
  const earliestDate = new Date(2023, 0, 1);

  const prevDay = subDays(currentDate, 1);
  const prevMonth = subMonths(currentDate, 1);
  const prevYear = subYears(currentDate, 1);

  const canGoBackDay = !isBefore(prevDay, earliestDate);
  const canGoBackMonth = !isBefore(prevMonth, earliestDate);
  const canGoBackYear = !isBefore(prevYear, earliestDate);
  ...
  const { data, isLoading } = useQuery<{
    ...
    queryKey: ['news', 'past', format(currentDate, 'dd/MM/yyyy'), page],
    queryFn: async () => {
      const response = await axios.get(`/api/news/past?date=${format(currentDate, 'dd/MM/yyyy')}&page=${page}`);
